...
Proof.
  intros n m.   (* move both quantifiers into the context forall n,m*)
  intros H.     (* move the hypothesis into the context n=m*)
  rewrite -> H. (* Rewrite the goal using the hypothesis m+m =m+m*)
  reflexivity.  Qed.
...
(* They result in the same divisions (n as [| n']) but induction is 
meant to define the base and step cases, while destruct can split 
in other ways. This results in two steps where the second can assume 
step k is true and k+1 must be shown. *)
